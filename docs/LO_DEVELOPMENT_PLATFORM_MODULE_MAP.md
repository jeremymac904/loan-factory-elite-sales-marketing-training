...
|--------|----------|---------|----------------|--------------|------------------|
| **Apex Advisor** | Paid coaching subscribers (Tier 1 $249/mo, Tier 2 $449/mo) | Power Hour, Breakfast Club, certifications, leaderboards, trackers, member area, mastermind, summit — the paid coaching layer on top of the 101–601 curriculum | `src/app/apex-advisor/`, `src/app/apex-advisor-track/`, `src/app/apex-advisor-pro/`, `src/app/apex-certifications/`, `src/app/apex-calendar/`, `src/app/apex-leaderboards/`, `src/app/apex-mastermind/`, `src/app/apex-launch-call/`, `src/app/apex-member-area/`, `src/data/apex.ts` | `/apex-advisor` (hub) plus `/apex-advisor-track`, `/apex-advisor-pro`, `/apex-certifications`, `/apex-calendar`, `/apex-leaderboards`, `/apex-mastermind`, `/apex-member-area` | **Built** |
| **Elite Sales & Marketing Training** | All Loan Factory LOs | The 101–601 curriculum: beginner, intermediate, advanced. Scripts, prompts, roleplays, trackers, recordings, audio briefs | `src/app/101-foundation/`, `src/app/201-borrower-conversion/`, `src/app/301-referral-partner-growth/`, `src/app/401-content-and-marketing/`, `src/app/501-pipeline-and-sales-systems/`, `src/app/601-elite-execution/`, `src/app/scripts/`, `src/app/prompts/`, `src/app/roleplays/`, `src/app/tracker/`, `src/app/recordings/`, `src/app/audio-training/`, `src/app/training-path/` | `/sales-training` (hub) plus `/sales-training/101` through `/sales-training/601` | **Built** (routes will be aliased/redirected from existing top-level paths) |
...
|---------|---------|----------------|--------------|
| Calendar | Unified events: Power Hour, Breakfast Club, training sessions, coaching calls, summit | `src/app/apex-calendar/` (Apex-only today) | `/calendar` (unified) and `/apex-calendar` (Apex-only sub-view) |
| Member Area | Gated home for all authenticated LOs (prototype: open) | `src/app/apex-member-area/` (Apex-only today) | `/member-area` (all LOs) and `/apex-member-area` (Apex tier) |
...
- **Built (2):** Apex Advisor, Elite Sales & Marketing Training
