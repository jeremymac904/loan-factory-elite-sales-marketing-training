...
**Suggested intro script:** "This is the Corporate Coach view. I'm previewing it through View-As so you can see the coaching workspace exactly as a coach signs in to it."
...
7. Click **Exit View-As**. Land URL: `/admin/view-as`.

## What to click

- The Coaching nav entry on `/`.
- One assigned LO card inside `/coaching` to show the per-LO detail.
- The Reports tile or `/coach-reports` link.
...
- Showing real LO names or session notes. Use seeded demo coaching data only.
- Skipping `/coach-guide` because it looks like static content — it is part of the daily flow.
- Recording from a Corporate Coach who has zero assignments. If the dashboard is empty, pick a different demo coach in View-As.

## Recording notes

- The orange banner must remain visible.
- If a real LO appears as a coachee, blur the name or scroll past.
- Click slowly so the viewer can read the report column headers.
- Keep the closing frame on `/coaching` before exiting View-As so the viewer remembers the hub.

**Suggested closing script:** "That is the Corporate Coach workspace. Exiting View-As now and moving to the Marketing role."
